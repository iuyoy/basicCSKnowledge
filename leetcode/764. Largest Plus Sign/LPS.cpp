...
public:
    int orderOfLargestPlusSign(int n, vector<vector<int>>& mines) {
        vvvi map(n, vvi(n, vi({1,0,0,0,0})));
        // map, left, right ,up, down;
        for(vi mine:mines)
            map[mine[0]][mine[1]][0] = 0;
        for(int i=1;i<n-1; ++i){
            for(int j=1; j<n-1; ++j){
                if(map[i][j-1][0]) map[i][j][1] = map[i][j-1][1]+1;
                if(map[i-1][j][0]) map[i][j][3] = map[i-1][j][3]+1;
                if(map[i][n-j][0]) map[i][n-1-j][2] = map[i][n-j][2]+1;
                if(map[n-i][j][0]) map[n-1-i][j][4] = map[n-i][j][4]+1;
            }
        }
        int ret = 0;
        for(int i=0; i<n; ++i)
            for(int j=0; j<n; ++j)
                if(map[i][j][0]) ret = max(ret, 1+min(min(map[i][j][1], map[i][j][2]), min(map[i][j][3], map[i][j][4])));
        return ret;
    }
};

// DP change vector with array.
class Solution {
public:
    int orderOfLargestPlusSign(int n, vector<vector<int>>& mines) {
        int map[n][n][5];
        // map, left, right ,up, down;
        memset(map, 0, sizeof(int)*n*n*5);
        for(vi& mine:mines)
            map[mine[0]][mine[1]][0] = 1;
        for(int i=1;i<n-1; ++i){
            for(int j=1; j<n-1; ++j){
                if(map[i][j-1][0] == 0) map[i][j][1] = map[i][j-1][1]+1;
                if(map[i-1][j][0] == 0) map[i][j][3] = map[i-1][j][3]+1;
                if(map[i][n-j][0] == 0) map[i][n-1-j][2] = map[i][n-j][2]+1;
                if(map[n-i][j][0] == 0) map[n-1-i][j][4] = map[n-i][j][4]+1;
            }
        }
        int ret = 0;
        for(int i=0; i<n; ++i)
            for(int j=0; j<n; ++j)
                if(map[i][j][0] == 0) ret = max(ret, 1+min(min(map[i][j][1], map[i][j][2]), min(map[i][j][3], map[i][j][4])));
        return ret;
    }
};
